...
export function ExecutiveCleanTemplate({ data }: { data: ResumeTemplateData }) {
  ...
  return (
    <article className="paper-panel mx-auto max-w-4xl rounded-2xl border border-slate-200 bg-white p-6 shadow-[0_16px_34px_rgba(15,23,42,0.07)] sm:p-8">
      <header className="border-b border-slate-900 pb-4">
        <h2 className="font-serif text-4xl font-bold tracking-tight text-slate-950 sm:text-5xl">{name}</h2>
        {role && role !== name ? <p className="mt-2 text-base font-medium text-slate-700">{role}</p> : null}
        {contact.length ? <p className="mt-2 text-sm text-slate-600">{contact.join(" • ")}</p> : null}
      </header>

      {text(data.content.summary) ? (
        <section className="mt-5">
          <h3 className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Executive Summary</h3>
      ...
      {data.content.workExperience.length ? (
        <section className="mt-6">
          <h3 className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Professional Experience</h3>
          <div className="mt-3 space-y-3">
            {data.content.workExperience.map((item) => {
              ...
              return (
                <article key={item.id} className="border-b border-slate-100 pb-3 last:border-b-0 last:pb-0">
      ...
      {data.content.projects.length ? (
        <section className="mt-6">
          <h3 className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Selected Projects</h3>
          <div className="mt-3 space-y-3">
      ...
      <section className="mt-6 grid gap-4 md:grid-cols-2">
        {data.content.education.length ? (
          <div>
            <h3 className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Education</h3>
        ...
        {data.content.skills.length ? (
          <div>
            <h3 className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Core Skills</h3>
            <p className="mt-2 text-sm leading-7 text-slate-800">{data.content.skills.map((skill) => text(skill)).filter(Boolean).join(", ")}</p>
          </div>
        ) : null}
      </section>
      ...
      {data.content.certifications.length ? (
        <section className="mt-6">
          <h3 className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Certifications</h3>
          <div className="mt-2 space-y-2">
